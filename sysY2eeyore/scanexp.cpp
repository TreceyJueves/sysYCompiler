...
std::pair<std::pair<int, int>, v_type> scanexp(basenode*, bool);
int scanconstexp(basenode*, bool);
std::pair<std::pair<int, int>, v_type> scanaddexp(basenode*, bool);
std::pair<std::pair<int, int>, v_type> scanmulexp(basenode*, bool);
std::pair<std::pair<int, int>, v_type> scanunaryexp(basenode*, bool);
std::pair<std::pair<int, int>, v_type> scanprimaryexp(basenode*, bool);
std::pair<std::pair<int, int>, v_type> scanlval(basenode*, bool);
std::pair<std::pair<int, int>, v_type> scannumber(basenode*, bool);
std::string scanaddop(basenode*, bool);
std::string scanmulop(basenode*, bool);
std::string scanunaryop(basenode*, bool);
void scanfuncrparams(basenode*, bool);
void scanlvalarray(basenode*, bool, std::vector<std::pair<std::pair<int, int>, v_type>>&);
...
std::pair<std::pair<int, int>, v_type> scanexp(basenode* root, bool isdec){
    auto s = root->Son;
    if(s.size() != 1){
        yyerror("Exp Error: wrong number of sons\n");
    }
    return scanaddexp(s[0], isdec);
}
int scanconstexp(basenode* root, bool isdec){
    auto s = root->Son;
    if(s.size() != 1){
        yyerror("Exp Error: wrong number of sons\n");
    }
    ...
    if(res.second != _const_var){
        yyerror("ConstExp Error: should be const value\n");
    }
    return res.first.second;
}
std::pair<std::pair<int, int>, v_type> scanaddexp(basenode* root, bool isdec){
    auto s = root->Son;
    ...
    if(s.size() == 1)
        ...
        yyerror("AddExp Error: wrong number of sons\n");
        return ret;
    }
}

std::string scanaddop(basenode* root, bool isdec){
    auto s = root->Son;
    if(s[0]->type == _add)
        return "+";
        ...
        yyerror("AddOp Error: wrong type of son\n");
        return "";
    }
}

std::string scanmulop(basenode* root, bool isdec){
    auto s = root->Son;
    if(s[0]->type == _mul)
        return "*";
        ...
        yyerror("MulOp Error: wrong type of son\n");
        return "";
    }
}

std::string scanunaryop(basenode* root, bool isdec){
    ...
    if(s[0]->type == _add)
        ...
        yyerror("UnaryOp Error: wrong type of son\n");
        return "";
    }
}

std::pair<std::pair<int, int>, v_type> scanmulexp(basenode* root, bool isdec){
    auto s = root->Son;
...
std::pair<std::pair<int, int>, v_type> scanunaryexp(basenode* root, bool isdec){
    auto s = root->Son;
    ...
    if(s.size() == 1){
        ...
        if(func_returnvoid.find(func_ID) == func_returnvoid.end()) {
            yyerror("UnaryExp Error: Function Undefined\n");
        ...
        if(func_returnvoid.find(func_ID) == func_returnvoid.end()) {
            ...
            if(!noprint && !isdec){
                fprintf(yyout, "param %d\n", lineNo);
                fprintf(yyout, "call f_sysy_%s\n", func_ID.c_str());
            }
            return std::make_pair(std::make_pair(0,0),_null);;
        }
        else if(func_returnvoid[func_ID] == true){
        ...
        yyerror("UnaryExp Error: wrong number of sons\n");
        return std::make_pair(std::make_pair(0,0),_null);
    }
}

void scanfuncrparams(basenode* root, bool isdec){
    auto s = root->Son;
    if(s.size() == 1){
        ...
        yyerror("FuncRParams Error: wrong number of sons\n");
    }
    return;
}

std::pair<std::pair<int, int>, v_type> scanprimaryexp(basenode* root, bool isdec){
    auto s = root->Son;
    ...
    if(s.size() == 3){
        ...
        if(s[0]->type == _number){
            ...
            if(lv.second == _const_var_array || lv.second == _var_array){
                ...
                //problem here
            //    fprintf(yyout, "lv.val: %d\n", lv.first.second);
                return lv;
            }
        }
        else{
            printf("%d\n", s[0]->type);
            yyerror("PrimaryExp Error: wrong type of son\n");
            return std::make_pair(std::make_pair(0,0), _null);
        }
    }
    else{
        yyerror("PrimaryExp Error: wrong number of sons\n");
        return std::make_pair(std::make_pair(0,0), _null);
    }
}

std::pair<std::pair<int, int>, v_type> scanlval(basenode* root, bool isdec){
    printf("scan lval\n");
    printf("type: %d\n", root->type);
...
void scanlvalarray(basenode* root, bool isdec, std::vector<std::pair<std::pair<int, int>, v_type>>& cur_lval_array){
    auto s = root->Son;
    ...
    if(s.size() == 0){
        ...
        yyerror("LValArray Error: wrong number of sons\n");
        return;
    }
}

std::pair<std::pair<int, int>, v_type> scannumber(basenode* root, bool isdec){
    auto s = root->Son;
    printf("scan number\n");
    printf("type: %d\n", root->type);
    printf("number of sons: %ld\n", s.size());
    if(s.size() == 1){
        ...
        yyerror("Number Error: wrong number of sons\n");
        return std::make_pair(std::make_pair(0,0), _null);
    }
}