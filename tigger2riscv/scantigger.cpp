...
void scantigger(basenode*);
void scanglobalvardecl(basenode*);
void scanfuncdef(basenode*);
void scanfunctionhead(basenode*);
void scanexpressions(basenode*);
void scanfunctionend(basenode*);
void scanexpression(basenode*);

void scanglobalvardecl(basenode* root){
    auto s = root->Son;
    if(s.size() == 2){
        auto v = s[0]->ID.c_str();
        auto n = s[1]->val;
...
void scanfuncdef(basenode* root){
    auto s = root->Son;
    if(s.size() == 3){
...
void scanfunctionhead(basenode* root){
    auto s = root->Son;
    if(s.size() == 3){
        auto v = s[0]->ID.c_str();
        auto v1 = v+2;
...
void scanexpressions(basenode* root){
    auto s = root->Son;
    if(s.size() == 1){
        scanexpression(s[0]);
    }
