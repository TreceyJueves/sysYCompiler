...
void scanexpression(e_basenode* root){
    ...
    if(s.size() == 5){//VARIABLE ASSIGN RightValue BinOp RightValue
        ...
        if(var_local == 0){//global variable
            ...
            std::string output1 = "store " + name_l + " " + std::to_string(var_place)+"\n";
            fprintf(e2tout, "%s",output1.c_str());
        }
        else{
        ...
        if(var_local == 0){//global variable
            ...
            std::string output1 = "store a0 " + std::to_string(var_place)+"\n";
            fprintf(e2tout, "%s",output1.c_str());
        }
        else{
    ...
    //unfinished
        if(s[0]->type == _if){//case if rightvalue logicop rightvalue goto label
            ...
            fprintf(e2tout, "%s", out3.c_str()); 
        		...
        		std::string out3 = "store "+tmp_name + " " + std::to_string(var_place_l) +"\n";
            	fprintf(e2tout, "%s", out3.c_str()); 
			}
            if(tmp_reg != 0)
                regs[tmp_reg] = 0;
    ...
    if(s.size() == 1){
        if(s[0]->type == _variable){
            ...
            if(value == 0)
                ...
                iszero = 0;
            if(iszero && right_oprand)
                return 0;
            int reg_n = get_reg();
            std::string reg_name = trans_reg(reg_n);
            if(iszero == 0 || (iszero == 1 && right_oprand == 0)){
