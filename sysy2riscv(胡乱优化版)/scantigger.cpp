...
void scanexpression(t_basenode* root){
    auto s = root->Son;
    int old_last = lastsent_sw;
    lastsent_sw = 0;
    if(s.size() == 1){//case enter label return
        if(s[0]->type == _enter){
            lastsent_sw = old_last;
            //do nothing
        }
        ...
        if(s[1]->type == _assign && s[4]->type == _reg){//case REG ASSIGN REG OP REG ENTER
            auto op = s[3]->ID;
            id2val[s[0]->ID] = -100000000;
            if(op == "+"){
                if(id2val.find(s[4]->ID)==id2val.end()||id2val[s[4]->ID] != 0)
                    fprintf(yyout, "\tadd %s, %s, %s\n", s[0]->ID.c_str(), s[2]->ID.c_str(), s[4]->ID.c_str());
                else{
                    id2val[s[0]->ID] = id2val[s[2]->ID];
                }
            }
            else if(op == "-")
                fprintf(yyout, "\tsub %s, %s, %s\n", s[0]->ID.c_str(), s[2]->ID.c_str(), s[4]->ID.c_str());
            else if(op == "*"){
                if(!(id2val.find(s[2]->ID) == id2val.end()) &&  id2val[s[2]->ID] == 4)
                    fprintf(yyout, "\tslli %s, %s, 2\n", s[0]->ID.c_str(), s[4]->ID.c_str());
                else if(!(id2val.find(s[4]->ID) == id2val.end()) &&  id2val[s[4]->ID] == 4)
                    fprintf(yyout, "\tslli %s, %s, 2\n", s[0]->ID.c_str(), s[2]->ID.c_str());
                else if(!(id2val.find(s[2]->ID) == id2val.end()) &&  id2val[s[2]->ID] == 2)
                    fprintf(yyout, "\tslli %s, %s, 1\n", s[0]->ID.c_str(), s[4]->ID.c_str());
                else if(!(id2val.find(s[4]->ID) == id2val.end()) &&  id2val[s[4]->ID] == 2)
                    fprintf(yyout, "\tslli %s, %s, 1\n", s[0]->ID.c_str(), s[2]->ID.c_str());
                else
                    fprintf(yyout, "\tmul %s, %s, %s\n", s[0]->ID.c_str(), s[2]->ID.c_str(), s[4]->ID.c_str());
            }
            else if(op == "/"){
                if(!(id2val.find(s[4]->ID) == id2val.end()) &&  id2val[s[4]->ID] == 2)
                    fprintf(yyout, "\tsrli %s, %s, 1\n", s[0]->ID.c_str(), s[2]->ID.c_str());
                else
        ...
        //    id2val[s[0]->ID] = -100000000;
            ...
            auto num = s[1]->val;
            id2val[s[2]->ID] = -100000000;
            if(num <= 511 && num >= -512){
    //            fprintf(yyout,"oldlast: %d, lastsent: %d\n", old_last, lastsent_sw);
                if((old_last != 1 && old_last != 2) || cur_stkplace != num*4 || cur_reg != s[2]->ID){
                    fprintf(yyout, "\tlw %s, %d(sp)\n", reg1, num*4);
                }
                if(old_last == 2 || old_last == 1)
                    lastsent_sw = 3;
                else
                    lastsent_sw = 0;
            }
        ...
        if(s[0]->type == _if){
            if(old_last == 1 || old_last == 3)
                lastsent_sw = 2;
        //    fprintf(yyout,"old_last: %d, last_sentsw: %d\n", old_last,lastsent_sw);
            auto op = s[2]->ID;
            auto reg1 = s[1]->ID.c_str();
            auto reg2 = s[3]->ID.c_str();
